...
func mockNewDposContext(db badger.IStorage) *avlmerkle.DposContext {
	dposContext, err := avlmerkle.NewDposContextFromProto(db, &avlmerkle.DposContextProto{})
	if err != nil {
		return nil
	}
	delegator := []byte{}
	...
	for i := 0; i < maxValidatorSize; i++ {
		addresses = append(addresses, common.Hex2Address(MockEpoch[i]))
	}
	dposContext.SetValidators(addresses)
	for j := 0; j < len(MockEpoch); j++ {
		delegators := common.Hex2Address(MockEpoch[j])
		delegator = delegators.Bytes()
		candidates := common.Hex2Address(MockEpoch[j])
		candidate = candidates.Bytes()
		dposContext.DelegateTrie().Update(append(candidate, delegator...), candidate)
		dposContext.CandidateTrie().Update(candidate, candidate)
		dposContext.VoteTrie().Update(candidate, candidate)
	}
	return dposContext
}

func setMintCntTrie(epochID int64, candidate common.Address, mintCntTrie *avlmerkle.Tree, count int64) {
	key := make([]byte, 8)
	binary.BigEndian.PutUint64(key, uint64(epochID))
	cntBytes := make([]byte, 8)
	binary.BigEndian.PutUint64(cntBytes, uint64(count))
	mintCntTrie.Update(append(key, candidate.Bytes()...), cntBytes)
}

func getMintCnt(epochID int64, candidate common.Address, mintCntTrie *avlmerkle.Tree) int64 {
	key := make([]byte, 8)
	binary.BigEndian.PutUint64(key, uint64(epochID))
	cntBytes, _ := mintCntTrie.Get(append(key, candidate.Bytes()...))
	if cntBytes == nil {
		return 0
	} else {
...
func TestUpdateMintCnt(t *testing.T) {
	...
	miner := common.Hex2Address("0xa60a3886b552ff9992cfcd208ec1152079e046c2")
