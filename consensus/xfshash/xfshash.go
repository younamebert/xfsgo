...
func (c *cache) generate(dir string, limit int, test bool) {
	c.once.Do(func() {
		...
		logger := logrus.New() // log.New("epoch", c.epoch)
...
// Ethash is a gite engine based on proot-of-work implementing the ethash
// algorithm.
type Ethash struct {
...
func New(cachedir string, cachesinmem, cachesondisk int, dagdir string, dagsinmem, dagsondisk int) *Ethash {
	if cachesinmem <= 0 {
		logrus.Warn("One ethash cache must always be in memory", "requested", cachesinmem)
		cachesinmem = 1
	}
	if cachedir != "" && cachesondisk > 0 {
		logrus.Info("Disk storage enabled for ethash caches", "dir", cachedir, "count", cachesondisk)
	}
	if dagdir != "" && dagsondisk > 0 {
		logrus.Info("Disk storage enabled for ethash DAGs", "dir", dagdir, "count", dagsondisk)
	}
	return &Ethash{
		cachedir:     cachedir,
...
func (ethash *Ethash) cache(block uint64) []uint32 {
	...
	if current == nil {
		...
		for len(ethash.caches) > 0 && len(ethash.caches) >= ethash.cachesinmem {
			...
			logrus.Trace("Evicted ethash cache", "epoch", evict.epoch, "used", evict.used)
		}
		// If we have the new cache pre-generated, use that, otherwise create a new one
		if ethash.fcache != nil && ethash.fcache.epoch == epoch {
			logrus.Trace("Using pre-generated cache", "epoch", epoch)
			current, ethash.fcache = ethash.fcache, nil
		} else {
			logrus.Trace("Requiring new ethash cache", "epoch", epoch)
		...
		if ethash.fcache == nil || ethash.fcache.epoch <= epoch {
			...
			logrus.Trace("Requiring new future ethash cache", "epoch", epoch+1)
...
func (ethash *Ethash) dataset(block uint64) []uint32 {
	...
	if current == nil {
		...
		for len(ethash.datasets) > 0 && len(ethash.datasets) >= ethash.dagsinmem {
			...
			logrus.Trace("Evicted ethash dataset", "epoch", evict.epoch, "used", evict.used)
		}
		// If we have the new cache pre-generated, use that, otherwise create a new one
		if ethash.fdataset != nil && ethash.fdataset.epoch == epoch {
			logrus.Trace("Using pre-generated dataset", "epoch", epoch)
			current = &dataset{epoch: ethash.fdataset.epoch} // Reload from disk
			ethash.fdataset = nil
		} else {
			logrus.Trace("Requiring new ethash dataset", "epoch", epoch)
		...
		if ethash.fdataset == nil || ethash.fdataset.epoch <= epoch {
			...
			logrus.Trace("Requiring new future ethash dataset", "epoch", epoch+1)
...
// that is empty.
func (ethash *Ethash) APIs(chain *xfsgo.BlockChain) error {
	return nil
}
