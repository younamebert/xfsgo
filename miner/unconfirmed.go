...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		switch {
		case header == nil:
			logrus.Warnf("Failed to retrieve header of mined block number %v hash %v", next.index, next.hash.Hex())
		case header.HashHex() == next.hash.Hex():
			logrus.Infof("block reached canonical chain number %v hash %v", next.index, next.hash.Hex())
		default:
			logrus.Infof("⑂ block  became a side fork number %v hash %v", next.index, next.hash.Hex())
		}
