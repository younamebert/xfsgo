...
func newTable(t transport, ourID NodeId, ourAddr *net.UDPAddr, nodeDBPath string, log log.Logger) *Table {
...
func (tab *Table) refresh() {
	...
	tab.mu.Unlock()
	//tab.log.Debugf("P2P discover table refresh doing: seed=%v", seed)
	...
	if seed {
		...
		//for i, see := range seeds {
		//tab.log.Debugf("P2P discover table refresh query seeds[%d]: node=%s", i,see)
		//}
...
func (tab *Table) bondall(nodes []*Node) (result []*Node) {
	...
	for i := range nodes {
		//tab.log.Debugf("P2P discover bondall: index=%d, node=%s",i, nodes[i])
		go func(n *Node) {
