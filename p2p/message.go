...
func (m *helloRequestMsg) String() string {
	return fmt.Sprintf(`type=%d, version=%d, id=%s, receiveId=%s`,
		typeHelloRequest, version1, m.id, m.receiveId)
}
...
func (m *helloReRequestMsg) String() string {
	return fmt.Sprintf(`type=%d, version=%d, id=%s, receiveId=%s`,
		typeHelloRequest, version1, m.id, m.receiveId)
}
