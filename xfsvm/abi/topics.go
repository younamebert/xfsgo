	...
	for i, filter := range query {
		for _, rule := range filter {
			...
			case common.Hash:
				copy(topic[:], rule[:])
			case common.Address:
				copy(topic[common.HashLen-common.AddrLen:], rule[:])
			case *big.Int:
				blob := rule.Bytes()
				copy(topic[common.HashLen-len(blob):], blob)
			case bool:
				if rule {
					topic[common.HashLen-1] = 1
				}
			case int8:
...
func genIntType(rule int64, size uint) []byte {
	var topic [common.HashLen]byte
	if rule < 0 {
		// if a rule is negative, we need to put it into two's complement.
		// extended to common.HashLength bytes.
		topic = [common.HashLen]byte{255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255}
	}
	for i := uint(0); i < size; i++ {
		topic[common.HashLen-i-1] = byte(rule >> (i * 8))
	}
	return topic[:]
}
